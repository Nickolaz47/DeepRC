...
See `deeprc/examples/` for some examples.
...
def compute_position_features(max_seq_len, sequence_lengths, dtype=np.float16):
    ...
    half_sequence_lengths = np.asarray(np.ceil(sequence_lengths / 2.0), dtype=np.int64)
    for i in range(len(sequences)):
        sequence, seq_len, half_seq_len = (
            sequences[i],
            sequence_lengths[i],
            half_sequence_lengths[i],
        )
        sequence[:seq_len, -1] = np.abs(0.5 - np.linspace(1.0, 0, num=seq_len)) * 2.0
        sequence[:half_seq_len, -3] = sequence[:half_seq_len, -1]
        sequence[half_seq_len:seq_len, -2] = sequence[half_seq_len:seq_len, -1]
        sequence[:seq_len, -1] = 1.0 - sequence[:seq_len, -1]
    return sequences
    ...
    def __init__(
        ...
        for i in range(self.n_layers):
            lstm = nn.LSTM(
                ...
                hidden_size=self.n_lstm_blocks,
                **lstm_kwargs,
            )
            network.append(lstm)
...
class AttentionNetwork(nn.Module):
    def __init__(self, n_input_features: int, n_layers: int = 2, n_units: int = 32, dropout_rate:float=0.0):
        ...
        for _ in range(self.n_attention_layers):
            ...
            att_linear.weight.data.normal_(
                0.0, np.sqrt(1 / np.prod(att_linear.weight.shape))
            )
...
class OutputNetwork(nn.Module):
    def __init__(self, n_input_features: int, n_output_features: int = 1, n_layers: int = 1, n_units: int = 32,
        ...
        for _ in range(self.n_layers - 1):
            ...
            o_linear.weight.data.normal_(
                0.0, np.sqrt(1 / np.prod(o_linear.weight.shape))
            )
...
class DeepRC(nn.Module):
    def __init__(
        ...
        consider_seq_counts: bool = False,
        add_positional_information: bool = True,
        sequence_reduction_fraction: float = 0.1,
        ...
        if sequence_embedding_as_16_bit:
            ...
            self.sequence_embedding = sequence_embedding_network.to(
                device=device, dtype=self.embedding_dtype
            )
        else:
        ...
        position_features = compute_position_features(
            max_seq_len=max_seq_len, sequence_lengths=np.arange(max_seq_len + 1)
        )
        self.position_features = (
            torch.from_numpy(position_features)
            .to(device=device, dtype=self.embedding_dtype)
            .detach()
        )

    def reduce_and_stack_minibatch(
        self, targets, sequences_of_indices, sequence_lengths, sequence_counts
    ):
        ...
        with torch.no_grad():
            ...
            inputs_list = [
                ...
                for sequence_of_indices, sequence_lengths, counts_per_sequence in zip(
                    sequences_of_indices, sequence_lengths, sequence_counts
                )
            ]
            ...
            reduced_inputs, reduced_sequence_lengths, used_sequence_inds = list(
                zip(
                    *[
                        ...
                        for inp, sequence_lengths in zip(inputs_list, sequence_lengths)
                    ]
                )
            )
            ...
            mb_n_sequences = torch.tensor(
                [len(rsl) for rsl in reduced_sequence_lengths],
                dtype=torch.long,
                device=self.device,
            )
    ...
    def __compute_features__(
        ...
        sequence_char_indices = sequence_char_indices.to(
        ...
        if self.consider_seq_counts:
            counts_per_sequence = counts_per_sequence.to(
                dtype=self.embedding_dtype, device=self.device
            )
        # Allocate tensor for one-hot sequence features + position features
        features_one_hot_shape = (
            sequence_char_indices.shape[0],
            max_mb_seq_len,
            n_features,
        )
        features_one_hot_padded = torch.zeros(
            size=features_one_hot_shape, dtype=self.embedding_dtype, device=self.device
        )
        ...
        features_one_hot[
            torch.arange(features_one_hot.shape[0]), sequence_char_indices.reshape((-1))
        ] = 1.0
        # Set padded sequence-parts to 0 (sequence_char_indices == -1 marks the padded positions)
        features_one_hot[sequence_char_indices.reshape((-1)) == -1, -1] = 0.0
        features_one_hot = features_one_hot.reshape(
            (sequence_char_indices.shape[0], sequence_char_indices.shape[1], n_features)
        )
        features_one_hot_padded[:, : sequence_char_indices.shape[1], :] = (
            features_one_hot
        )
        # Scale by sequence counts
        if self.consider_seq_counts:
            features_one_hot_padded = (
                features_one_hot_padded * counts_per_sequence[:, None, None]
            )
        # Add position information
        if self.add_positional_information:
            features_one_hot_padded[:, : sequence_char_indices.shape[1], -3:] = (
                self.position_features[
                    sequence_lengths, : sequence_char_indices.shape[1]
                ]
            )
        # Perform normalization to std=1
        features_one_hot_padded = (
            features_one_hot_padded / features_one_hot_padded.std()
        )
    ...
    def __reduce_sequences_for_bag__(self, inputs, sequence_lengths):
        ...
        if self.sequence_reduction_fraction <= 1.0:
            ...
            n_reduced_sequences = int(
                sequence_lengths.shape[0] * self.sequence_reduction_fraction
            )
            ...
            for mb_i in mb_is.unbind(dim=0):
                ...
                inputs_mb = inputs[
                    mb_i * self.reduction_mb_size : (mb_i + 1) * self.reduction_mb_size
                ].to(device=self.device, dtype=self.embedding_dtype)
                sequence_lengths_mb = sequence_lengths[
                    mb_i * self.reduction_mb_size : (mb_i + 1) * self.reduction_mb_size
                ].to(device=self.device, dtype=torch.long)

                # Get sequence embedding (h())
                emb_seqs = self.sequence_embedding(
                    inputs_mb, sequence_lengths=sequence_lengths_mb
                ).to(dtype=torch.float32)
            ...
            _, used_sequences = torch.topk(
                attention_acts, n_reduced_sequences, dim=0, largest=True, sorted=True
            )

            # Get top k sequences and sequence lengths
            reduced_inputs = (
                inputs[used_sequences.to(device=self.device)]
                .detach()
                .to(device=self.device, dtype=self.embedding_dtype)
            )
            reduced_sequence_lengths = (
                sequence_lengths[used_sequences.to(device=self.device)]
                .detach()
                .to(device=self.device, dtype=self.embedding_dtype)
            )
        else:
            with torch.no_grad():
                reduced_inputs = inputs.detach().to(
